  ...
  export default {
    ...
    methods: {
      ...
      async update() {
        ...
        let MyJSON = JSON.stringify({newPassword: this.editedAmuta.newPassword,userId: this.$route.params.amutObj.id});
        let token = localStorage.getItem("token");

        let api = this.host + "/admin/resetPassword"
        ...
        return  res.json()
          ...
          this.showSnackBar("הסיסמה עודכנה בהצלחה!", "green");
          setTimeout(() => {
            this.$router.push({ name: 'superManager'})
          }, 700);
        }).catch((error) => {
